        ...
        return promocionesCaducadas.map((promocion) => {
          return (
            <PromocionEdit
              eventos={eventos}
              id={promocion.id}
              onClick={() => handleShowPromo(promocion.id)}
              key={promocion.id}
              caducado={true}
              titulo={promocion.titulo}
              comercio={promocion.comercio_nombre}
              evento={promocion.evento_id}
              descripcion={promocion.descripcion}
              inicio={promocion.fecha_inicio}
              src={promocion.src}
              final={promocion.fecha_expiracion}
            />
          );
        });
      case 2:
        ...
        return promocionesCaducadas.map((promocion) => {
          return (
            <Promocion
              ...
              titulo={promocion.titulo}
              comercio={promocion.comercio_nombre}
              evento={promocion.evento_nombre}
              descripcion={promocion.descripcion}
              inicio={promocion.fecha_inicio}
              src={promocion.src}
              final={promocion.fecha_expiracion}
  ...
  useEffect(() => {
    ...
    async function getPromociones() {
      try {
        ...
        if (user.rol === 4) {
          let eventos = await EventsService.index("eventos");
          console.log("eventos", eventos);
          if (eventos.status === 200) {
            setEventos(eventos.data);
          }
          let userPromociones =
            await PromocionesService.getPromocionesByComercio(user.comercio_id);
          let userPromocionesCaducadas =
            await PromocionesService.getPromocionesExpiredByUser(user.id);
        ...
        let userPromociones = await PromocionesService.getPromocionesByUser(
          user.id
        );
        const userPromocionesCaducadas =
          await PromocionesService.getPromocionesExpiredByUser(user.id);
  ...
  return (
    <div>
      <Filters
        ...
        selected={selected}
        setPromociones={setFilteredPromociones}
        filteredPromociones={filteredPromociones}
