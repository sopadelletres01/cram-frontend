...
const Event = () => {
  ...
  const renderPromociones = () => {
    return promociones.map((promo) => {
      return (
        <div
          className=" hover-zoom rounded p-2"
          style={{
            display: "flex",
            gap: "16px",
            alignItems: "center",
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await EventsService.show("eventos", id);
        console.log("res", res);
        if (res.status === 200) {
          setEventData(res.data);
        }
        const userEventos = await EventsService.getEventosByUser(user.id);
        console.log("caducadas", userEventos);
        console.log("resdata", res.data);
        const cositas = userEventos.data.find(
          (ev) => ev.id_evento === res.data.id
        );
        console.log("cositas", cositas);
        if (cositas) {
          setEventData({ ...res.data, inscrito: true });
        }
        const comercios = await EventsService.getComercios(id);
        console.log("comercios", comercios);
        if (comercios.status === 200) {
          setComercios(comercios.data);
        }
        const promociones = await EventsService.getPromociones(id);
  ...
  return (
    <Card
      style={{
        ...
        background:
          "linear-gradient(180deg, rgba(108,172,255,1) 17%, rgba(141,235,255,1) 89%)",
      }}
    >
      <Card.Body>
        <div
        ...
        >
          ...
          >
            <Card.Img
              style={{ flex: "1", maxHeight: "600px", maxWidth: "600px" }}
              variant="top"
              src={eventData.src}
            />
            <div
            ...
            >
              ...
              <span>
                <strong>El evento se celebra el dia :</strong>{" "}
                <span>{eventData.fecha_inicio}</span>
              </span>
              <span>
